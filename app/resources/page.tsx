...
export const metadata: Metadata = createPageMetadata({
  title: "NZ Moving and Move-Out Cleaning Guides",
  description:
    "Use practical New Zealand moving and move-out cleaning guides, cost information, checklists, an interactive calculator, and quote-comparison advice.",
  path: "/resources",
});
...
export default function ResourcesPage() {
  const movingCards = resourceCards.filter((resource) => resource.category !== "cleaning");
  const cleaningCards = resourceCards.filter((resource) => resource.category === "cleaning");

  const structuredData = {
    "@context": "https://schema.org",
    "@type": "CollectionPage",
    name: "New Zealand moving and cleaning resources",
    description:
      "Moving and move-out cleaning guides, calculators, checklists, regional advice, and quote-comparison guidance from Match 'n Move.",
    ...
    <SiteShell>
      ...
      <section className="relative overflow-hidden bg-[radial-gradient(circle_at_top_left,rgba(56,189,248,0.2),transparent_28%),radial-gradient(circle_at_85%_15%,rgba(34,197,94,0.16),transparent_24%),linear-gradient(145deg,#071525,#12345d)] py-12 text-white sm:py-16 lg:py-20">
        <div className="container-shell relative">
          ...
          <h1 className="mt-5 max-w-[14ch] text-[clamp(2.6rem,9vw,5.6rem)] font-black leading-[0.95] tracking-[-0.055em] text-white">
            Plan the move and the clean in one place.
          </h1>
          <p className="mt-5 max-w-3xl text-base leading-8 text-slate-200 sm:text-lg">
            Explore practical New Zealand moving-cost guidance, cleaning checklists, end-of-tenancy advice,
            an interactive budget calculator, and clear ways to compare quotes on equal terms.
          </p>
      ...
      <section className="bg-[linear-gradient(180deg,#eef5fb_0%,#ffffff_100%)] py-12 sm:py-16">
        <div className="container-shell">
          <ResourceGroup
            id="moving-guides"
            eyebrow="Moving guides and tools"
            title="Plan and compare your move"
            description="Cost guides, regional planning advice, checklists, and tools for clearer mover quotes."
            resources={movingCards}
          />

          <ResourceGroup
            id="cleaning-guides"
            eyebrow="Move-out cleaning guides"
            title="Prepare the property for handover"
            description="Substantial, practical guidance for end-of-tenancy cleaning, cleaner quotes, room-by-room planning, and coordinating cleaners with moving day."
            resources={cleaningCards}
            cleaning
          />
        </div>
      </section>
...
function ResourceGroup({
  ...
  return (
    <section id={id} className="scroll-mt-24 py-7 first:pt-0 last:pb-0 sm:py-10">
      ...
      <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
        {resources.map((resource) => {
          const Icon = cleaning
            ? Home
            : resourceIcons[resource.slug as keyof typeof resourceIcons] ?? FileSearch;
          ...
          return (
            ...
            >
              ...
              <h3 className="mt-2 text-2xl font-black tracking-[-0.04em] text-slate-950">{resource.title}</h3>
