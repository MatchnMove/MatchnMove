...
import { getRuntimeErrorMessage, logRuntimeWarning } from "@/lib/runtime-errors";
...
export async function GET() {
  try {
    ...
    logRuntimeWarning("Admin MFA setup unavailable", error);
    return NextResponse.json({ error: `Admin MFA setup failed: ${getRuntimeErrorMessage(error)}` }, { status: 500 });
  }
}
