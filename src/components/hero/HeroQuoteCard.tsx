...
export function HeroQuoteCard() {
  ...
  return (
    <form
      onSubmit={submit}
      className="relative z-20 w-full overflow-visible rounded-[20px] border border-white/10 bg-[linear-gradient(145deg,#101b34_0%,#172857_64%,#1f3f86_100%)] p-3 text-left text-white shadow-[0_20px_48px_-36px_rgba(15,23,42,0.85)] sm:p-3.5"
    >
      <p className="max-w-[32rem] text-[0.82rem] leading-[1.45] text-slate-200 sm:text-[0.88rem]">
        Tell us about your move once, review transparent options, and choose the best mover for your timeline.
      </p>

      <h2 className="mt-2 text-[1.12rem] font-black leading-tight tracking-[-0.03em] text-white sm:text-[1.28rem]">
      ...
      <div className="mt-2.5 grid gap-2 sm:grid-cols-2">
        <AddressAutocomplete
          ...
          error={errors.from}
          labelClassName="mb-1 block text-[0.82rem] font-semibold text-slate-100"
          inputClassName={`w-full rounded-2xl border bg-white py-2 pr-4 pl-9 text-[0.9rem] text-slate-900 placeholder:text-slate-400 shadow-sm transition duration-150 focus:outline-none ${
      ...
      <div className="mt-2 flex">
        <button
          type="submit"
          className="inline-flex min-h-[42px] w-full items-center justify-center rounded-2xl bg-[linear-gradient(135deg,#5468ee,#6171f3)] px-5 py-2 text-[0.95rem] font-bold text-white shadow-[0_18px_36px_-22px_rgba(79,100,235,0.72)] transition duration-200 hover:translate-y-[-1px] hover:shadow-[0_22px_42px_-22px_rgba(79,100,235,0.82)] focus:outline-none focus:ring-4 focus:ring-brandBlue/20 active:translate-y-0 sm:w-auto sm:min-w-[174px]"
        >
          Get free quotes
        </button>
      </div>
